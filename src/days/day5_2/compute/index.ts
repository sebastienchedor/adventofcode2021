...
export function compute(input: InputModel): number {
	...
	for (let i = 0; i < input.length; i += 2) {
		...
		if (x1 === x2 || y1 === y2) {
			if (x1 === x2 && y1 < y2) {
				for (let i = y1; i <= y2; i++) covering_points.push(`${x1},${i}`);
			}

			if (x1 === x2 && y2 < y1) {
				for (let i = y2; i <= y1; i++) covering_points.push(`${x1},${i}`);
			}

			if (x1 < x2 && y2 === y1) {
				for (let i = x1; i <= x2; i++) covering_points.push(`${i},${y1}`);
			}

			if (x2 < x1 && y2 === y1) {
				for (let i = x2; i <= x1; i++) covering_points.push(`${i},${y1}`);
			}
		}
		...
		if (x1 < x2 && y1 < y2) {
			for (let i = x1; i <= x2; i++) xValues.push(i);
			for (let j = y1; j <= y2; j++) yValues.push(j);
			xValues.map((xVal, i) => {
				const yVal = yValues[i];
				covering_points.push(`${xVal},${yVal}`);
			});
		}
