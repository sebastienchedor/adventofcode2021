...
export function compute(input: InputModel): number {
  // Enlarge maze
  const new_input: InputModel = [];
  for (let index_line_maze = 0; index_line_maze < 5; index_line_maze++) {
    for (
      let index_column_maze = 0;
      index_column_maze < 5;
      index_column_maze++
    ) {
      for (
        let index_small_line_maze = 0;
        index_small_line_maze < input.length;
        index_small_line_maze++
      ) {
        if (index_column_maze === 0) new_input.push([]);
        for (
          let index_small_column_maze = 0;
          index_small_column_maze < input[0].length;
          index_small_column_maze++
        ) {
          const new_value =
            input[index_small_line_maze][index_small_column_maze].value +
            index_line_maze +
            index_column_maze;
          new_input[index_line_maze * input.length + index_small_line_maze][
            index_column_maze * input[0].length + index_small_column_maze
          ] = {
            distance_from_start: null,
            distance_to_end: null,
            value: new_value > 9 ? new_value - 9 : new_value,
          };
        }
      }
    }
